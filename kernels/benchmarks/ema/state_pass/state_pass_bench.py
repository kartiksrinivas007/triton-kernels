...
def bench_ema(BATCH, SEQLEN, HEAD_DIM, MAMBA_HEAD_DIM, MAMBA_CHUNK_SIZE, provider, device, raw, plot_time):
    ...
    if not raw:
        ...
        mamba_states = torch.randn((BATCH, NUM_CHUNKS, MAMBA_NUM_HEADS, MAMBA_HEAD_DIM), device=device, dtype=torch.float32)
        mamba_cs_last = torch.randn((BATCH, MAMBA_NUM_HEADS, NUM_CHUNKS), device = device, dtype=torch.float32)
        ema_cs_last = torch.randn((BATCH, NUM_CHUNKS), device=device, dtype=torch.float32)
        ema_states = torch.randn((BATCH, NUM_CHUNKS, HEAD_DIM), device = device, dtype=torch.float32)

        ema_args = ema_states, ema_cs_last
        mamba_args = mamba_states, mamba_cs_last


        mamba_data_read = size_computer(*mamba_args)
        ema_data_read  = size_computer(*ema_args)

        # write teh new final states per chunk
        ema_or_mamba_data_out = BATCH * NUM_CHUNKS * HEAD_DIM * 1 * ema_cs_last.element_size()
        ...
        def mamba_raw_fn():
            # get the outputs
            states, final_state = _state_passing_fwd(mamba_states, mamba_cs_last, out_dtype=torch.float32)

        def ema_raw_fn():
            # across heads the computation should be the same
            states, final_state = _ema_state_passing_fwd(ema_states, ema_cs_last, out_dtype=torch.float32)
...
if __name__ == "__main__":
    bench_ema.run(print_data=True, save_path="./kernels/benchmarks/ema/state_pass/state_pass_outputs/")